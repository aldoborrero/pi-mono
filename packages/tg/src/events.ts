...
import type { TelegramBot, TelegramEvent } from "./telegram.js";
...
export type TgEvent = ImmediateEvent | OneShotEvent | PeriodicEvent;
...
export class EventsWatcher {
	...
	/**
	 * Start watching for events. Call this after TelegramBot is ready.
	 */
	start(): void {
	...
	private parseEvent(content: string, filename: string): TgEvent | null {
	...
	private execute(filename: string, event: TgEvent, deleteAfter: boolean = true): void {
		// Format the message
		...
		const enqueued = this.telegram.enqueueEvent(syntheticEvent);
 ...
 */
export function createEventsWatcher(workspaceDir: string, telegram: TelegramBot): EventsWatcher {
	const eventsDir = join(workspaceDir, "events");
	return new EventsWatcher(eventsDir, telegram);
}
