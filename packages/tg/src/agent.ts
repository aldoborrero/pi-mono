...
import { createTgTools, setUploadFunction } from "./tools/index.js";
...
async function getProviderApiKey(provider: string, authStorage: AuthStorage): Promise<string> {
	...
	// Then try auth storage
	const key = await authStorage.getApiKey(provider);
	if (!key) {
		throw new Error(
			`No API key found for ${provider}.\n\n` +
				`Set the appropriate API key environment variable, or use /login with ${provider} and link to auth.json from ` +
				join(homedir(), ".pi", "tg", "auth.json"),
		);
	}
	return key;
}
...
function loadTgSkills(chatDir: string, workspacePath: string): Skill[] {
	const skillMap = new Map<string, Skill>();

	// chatDir is the host path (e.g., /Users/.../data/123456789)
	// hostWorkspacePath is the parent directory on host
	// workspacePath is the container path (e.g., /workspace)
	const hostWorkspacePath = join(chatDir, "..");
	...
	// Load chat-specific skills (override workspace skills on collision)
...
- Be careful with system modifications`;
	...
	return `You are pi-tg, a Telegram bot assistant. Be concise. No emojis.
...
\`\`\`json
{"type": "one-shot", "chatId": "${chatId}", "text": "Remind user about dentist", "at": "2025-12-15T09:00:00+01:00"}
\`\`\`
